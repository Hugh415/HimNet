...
def determine_motif_relation(mol, motif_i, motif_j):
    for atom_i in motif_i:
        for atom_j in motif_j:
            bond = mol.GetBondBetweenAtoms(atom_i, atom_j)
            if bond is not None:
                bond_type = bond.GetBondType()
                if bond_type == Chem.BondType.SINGLE:
                    return MOTIF_RELATION_COVALENT
                elif bond_type == Chem.BondType.DOUBLE:
                    return MOTIF_RELATION_COVALENT
                elif bond_type == Chem.BondType.TRIPLE:
                    return MOTIF_RELATION_COVALENT
                elif bond_type == Chem.BondType.AROMATIC:
                    return MOTIF_RELATION_PIPI
    
    min_distance = float('inf')
    for atom_i in motif_i:
        for atom_j in motif_j:
            pos_i = mol.GetConformer().GetAtomPosition(atom_i)
            pos_j = mol.GetConformer().GetAtomPosition(atom_j)
    ...
    if min_distance < 3.0:
        return MOTIF_RELATION_HYDROGEN
    elif min_distance < 5.0:
        return MOTIF_RELATION_NEARBY
    
    return MOTIF_RELATION_NONE
...
class BatchMolGraph:
    def __init__(self, smiles, atom_fdim, bond_fdim, fp_fdim, data_name):
        ...
        data_file = open(f'./data/{data_name}/raw/process_all.pkl', 'rb')
        ...
        self.motif_scopes = []
        self.motif_pair_indices = []
        self.motif_atom_indices = []

        self.motif_pair_relations = {}
        for mol_idx, mol_graph in enumerate(mol_graphs):
            if hasattr(mol_graph, 'cliques'):
                for i, clique_i in enumerate(mol_graph.cliques):
                    for j, clique_j in enumerate(mol_graph.cliques):
                        if i != j:
                            global_i = motif_offset + i
                            global_j = motif_offset + j
                            
                            relation = determine_motif_relation(mol_graph.mol, clique_i, clique_j)
                            self.motif_pair_relations[(global_i, global_j)] = relation
        
        motif_offset = 0
        for i, smi in enumerate(smiles):
            mol_graph = mol_graphs[smi[0]]
            
            num_motif = int(mol_graph.num_part[0][1])
            if num_motif > 0:
                self.motif_scopes.append((motif_offset, num_motif))
                
                atom_start = self.a_scope[i][0]
                for clique in mol_graph.cliques:
                    self.motif_atom_indices.append([atom_start + atom_idx for atom_idx in clique])
                
                for k1 in range(num_motif):
                    for k2 in range(k1+1, num_motif):
                        for bond_key, motif_pair in mol_graph.breaks_bonds.items():
                            if (motif_pair[0] == k1 and motif_pair[1] == k2) or \
                               (motif_pair[0] == k2 and motif_pair[1] == k1):
                                self.motif_pair_indices.append((
                                    motif_offset + k1, 
                                    motif_offset + k2
                                ))
                                break
                
                motif_offset += num_motif
