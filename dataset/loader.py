...
from dataset import atom_features, bond_features, get_fp_feature, motif_decomp
...
class HiMolGraph:
    ...
    def __init__(self, mol: Union[str, Chem.Mol]):
        ...
        '''
        Primitive pair feature extraction
        '''
        motif_pair_features = []
        motif_pair_indices = []
        if num_motif > 0:
            for k1, motif1 in enumerate(cliques):
                for k2, motif2 in enumerate(cliques):
                    if k1 < k2:
                        connected = False
                        for bond_key in breaks_bonds:
                            if (breaks_bonds[bond_key][0] == k1 and breaks_bonds[bond_key][1] == k2) or \
                            (breaks_bonds[bond_key][0] == k2 and breaks_bonds[bond_key][1] == k1):
                                connected = True
                                break
                        
                        if connected:
                            pair_feat = np.concatenate([motif_x[k1], motif_x[k2]])
                            motif_pair_features.append(pair_feat)
                            motif_pair_indices.append((k1, k2))

        self.motif_pair_features = torch.tensor(np.array(motif_pair_features)) if motif_pair_features else None
        self.motif_pair_indices = motif_pair_indices
        self.breaks_bonds = breaks_bonds
        self.cliques = cliques

        self.n_atoms = len(x)
        self.f_atoms = np.array(x).tolist()
        self.f_bonds = np.array(edge_attr).tolist()
        self.a2b = a2b
        self.b2a = b2a
        self.b2revb = b2revb
...
def _load_estrogen_dataset(input_path):
    """
...
def _load_metstab_dataset(input_path):
    """
...
def _load_cyp450_dataset(input_path):
    """
    :param input_path:
    :return: list of smiles, list of rdkit mol obj, np.array containing the
    labels (multiple classification tasks)
    """
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['smiles']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
...
def _load_lmc_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['smiles'].tolist()
    labels = input_df['hlm_clearance[mL.min-1.g-1]'].values.astype(np.float32)
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]

    valid_idx = [
        i for i, (mol, label) in enumerate(zip(rdkit_mol_objs_list, labels))
        if (mol is not None) and np.isfinite(label)
    ]
    smiles_list = [smiles_list[i] for i in valid_idx]
    rdkit_mol_objs_list = [rdkit_mol_objs_list[i] for i in valid_idx]
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        elif self.dataset == 'estrogen':
            smiles_list, rdkit_mol_objs, labels = \
                _load_estrogen_dataset(self.raw_paths[0])
            himol_graph = {}
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
        ...
        elif self.dataset == 'metstab':
            smiles_list, rdkit_mol_objs, labels = \
                _load_metstab_dataset(self.raw_paths[0])
            himol_graph = {}
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
        ...
        elif self.dataset == 'cyp450':
            smiles_list, rdkit_mol_objs, labels = \
                _load_cyp450_dataset(self.raw_paths[0])
            himol_graph = {}
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
        ...
        elif self.dataset == 'lmc':
            smiles_list, rdkit_mol_objs, labels = \
                _load_lmc_dataset(self.raw_paths[0])
            himol_graph = {}
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
                himol_graph[smiles_list[i]] = HiMolGraph(rdkit_mol)
